...
4. Cloudflare returns an address from the customer's space that was previously configured from a BYOIP space provided by the customer. In this case, the response was `152.3.15.200`, which is a part of the `/24` prefix of `152.3.15.0/24`.
5. The eyeball sends a request to `152.3.15.200` which is routed to Cloudflare.
